...
def _response(verification_id: UUID, result) -> ExternalVerificationResponse:
    return ExternalVerificationResponse(verification_id=verification_id, status=result.status.value, provider=result.provider, provider_version=result.provider_version, reason=result.reason, query_reference=result.query_reference, response_timestamp=result.response_timestamp, demo=result.demo)

@router.post("/{verification_id}/external/verify", response_model=ExternalVerificationResponse)
def verify(verification_id: UUID, request: ExternalVerifyRequest, user: AuthUser = Depends(require_permission(Permission.VERIFICATION_WORKFLOW)), db: Session = Depends(get_db)) -> ExternalVerificationResponse:
    document = SqlAlchemyIntelligenceRepository(db).get_document_for_verification_owner(verification_id, user.id)
    if document is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Verification not found.")
    query = ExternalVerificationQuery(request.document_type, request.document_number, request.country_code)
    return _response(verification_id, _provider().verify(query))
