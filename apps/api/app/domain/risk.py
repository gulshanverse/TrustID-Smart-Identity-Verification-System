...
def assess_risk(verification_id: UUID, validation: DocumentValidationResult, tampering: TamperingResult, face: FaceVerificationResult, ocr_confidence: float) -> RiskAssessmentResult:
    ...
    ocr_contribution = 0 if ocr_confidence >= 0.8 else 10
    factors.append(RiskFactor("OCR confidence", "ocr", RiskSeverity.INFO if ocr_contribution == 0 else RiskSeverity.REVIEW, ocr_contribution, "Structured OCR confidence is within the configured demo range." if ocr_contribution == 0 else "Structured OCR confidence is below the configured review threshold."))
    ...
    factors.append(RiskFactor("Technical document signals", "tampering", RiskSeverity.INFO if tampering_contribution == 0 else RiskSeverity.REVIEW if tampering_contribution < 18 else RiskSeverity.HIGH, tampering_contribution, "No suspicious technical tampering signal was detected." if tampering_contribution == 0 else "Technical document inconsistency detected; officer review is recommended.", str(tampering.id)))
    face_mapping = {FaceOutcome.MATCH: (0, RiskSeverity.INFO, "Face comparison matched within configured demo/provider criteria."), FaceOutcome.REVIEW: (12, RiskSeverity.REVIEW, "Face comparison is inconclusive and requires officer review."), FaceOutcome.MISMATCH: (25, RiskSeverity.HIGH, "Presented face comparison did not meet the configured threshold; this is not proof of fraud."), FaceOutcome.UNAVAILABLE: (10, RiskSeverity.REVIEW, "Face comparison was unavailable or failed quality conditions.")}
