...
Phase 2.4 adds a separate immutable baseline snapshot, a development-fold detector/quality sweep, a bounded input analysis, and a complete 6,000-pair rerun. The selected YuNet candidate uses confidence `0.90`, the existing 6,400-pixel face-size gate, existing brightness/contrast gates, existing box padding `0.0`, and a candidate blur threshold of `5.0`. The candidate is not the production default and was selected before the untouched full rerun.
